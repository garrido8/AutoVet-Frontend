...
export class AppointmentInfoComponent implements OnInit, OnDestroy {
  ...
  public shareForm = this.fb.group({
    permission: ['readonly', Validators.required], // Default to backend value
  });

  ngOnInit(): void {
    this.user =
      this.userInfoService.getFullStaffToken() ||
      this.userInfoService.getFullClientToken();
    this.staff = this.userInfoService.getFullStaffToken();
    const id = parseInt(this.route.snapshot.paramMap.get('id')!);

    this.authService.getStaffMembers().subscribe((response) => {
      if (response) {
        this.possibleColaborators = response.filter(
          (staff) =>
            staff.pk !== this.staff?.pk && !staff.email.includes('admin')
        );
      }
    });
  ...
  private loadRelatedData(appointment: Appoinment): void {
    const petNameSub = this.petService
      .getPetById(appointment.mascota)
      .subscribe((petResponse) => {
        if (petResponse) this.petName = petResponse.nombre;
      });
    this.subscriptions.add(petNameSub);

    const messagesSub = this.appointmentMessageService
      .getMessagesByAppointment(appointment.pk!)
      .subscribe((messages) => {
    ...
    if (appointment.fecha_resolucion) {
      this.fechaResolucionInput = this._formatDateForInput(
        appointment.fecha_resolucion
      );
      this.form.controls.fecha_resolucion.setValue(
        new Date(appointment.fecha_resolucion)
      );
    }
  }
  ...
  public openShareModal(): void {
    ...
    this.shareForm.reset({ permission: 'readonly' }); // Reset form to default backend value
  ...
  public closeModal(): void {
    const shouldNavigate =
      this.modalMessage.includes('modificada correctamente') ||
      this.modalMessage.includes('reasignación enviada') ||
      this.modalMessage.includes('compartida');
    this.showModal = false;
    if (shouldNavigate && !this.modalMessage.includes('Error')) {
      if (
        this.modalMessage.includes('modificada') ||
        this.modalMessage.includes('reasignación')
      ) {
  ...
  private messageUser(): string {
    ...
    return this.user.email.includes('autovet')
      ? `${this.user!.name} (Trabajador)`
  ...
  public onSendMessage(): void {
    if (this.messageForm.valid && this.appointment?.pk) {
      ...
      const messageSub = this.appointmentMessageService
  ...
  public onSubmit(): void {
    if (this.form.valid && this.appointment) {
      ...
      const editSub = this.appointmentService
        .editAppoinment(this.appointment.pk!, appointmentToUpdate)
        .subscribe({
          next: () => {
  ...
  public onReassignmentSubmit(): void {
    if (this.reassignmentForm.valid && this.appointment && this.staff) {
      ...
      const reassignmentSub = this.reassignmentService
        .addReassignment(reassignment)
        .subscribe({
          next: () => {
